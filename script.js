// Molina Metales — XAG fijo (podés usar ?asset=xau para ver Oro)
(function(){
  const defaultAsset = 'xag'; // ESTE repo es Plata
  const qs = new URLSearchParams(location.search);
  const asset = (qs.get('asset')||defaultAsset).toLowerCase()==='xag'?'xag':'xau';
  ...
  function recalc(){
    ...
    if(isNaN(base)){
      outSpot.textContent=out_g.textContent=out_ozt.textContent=out_kg.textContent=out_10g.textContent='—';
      out_calc.textContent='—'; return;
    }
  ...
  function mountTV(a){
    ...
    new TradingView.MiniSymbolOverview({
      container_id:symbolsId, symbols:[[symbol,""]], gridLineColor:"rgba(42,42,42,0.4)",
      dateRange:"12M", height:"100%", width:"100%", locale:"es", colorTheme:"dark"
    });
  }

  function setActive(a){
    tabXAU.classList.toggle('active', a==='xau');
    tabXAG.classList.toggle('active', a==='xag');
    ...
    updated.textContent = 'Actualizado — ' + nowStamp();
    mountTV(a);
  }

  tabXAU.addEventListener('click', ()=>{ const u=new URL(location); u.searchParams.set('asset','xau'); history.replaceState(null,'',u); setActive('xau'); });
  tabXAG.addEventListener('click', ()=>{ const u=new URL(location); u.searchParams.set('asset','xag'); history.replaceState(null,'',u); setActive('xag'); });

  let timer=null;
  autorefresh.addEventListener('change', ()=>{
    if(timer){ clearInterval(timer); timer=null; }
    const s=parseInt(autorefresh.value||'0',10);
    if(s>0){ timer=setInterval(()=>{ mountTV((new URLSearchParams(location.search).get('asset')||defaultAsset)); updated.textContent='Actualizado — ' + nowStamp(); }, s*1000); }
  });
  btnRefresh.addEventListener('click', ()=>{ mountTV((new URLSearchParams(location.search).get('asset')||defaultAsset)); updated.textContent='Actualizado — ' + nowStamp(); });
  btnCopy.addEventListener('click', async ()=>{ try{ await navigator.clipboard.writeText(location.href); btnCopy.textContent='Copiado'; setTimeout(()=>btnCopy.textContent='Copy URL',1200);}catch(e){} });

  // Defaults
  setActive(asset);
  spotUSD.value = (asset==='xau'?4017.56:50.00);
  usdars.value = 1200;
  recalc();
})();
